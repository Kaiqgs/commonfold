...
float loopNoise3d(vec2 val, float time, float loopDuration, float speed) {
    ...
    float v1 = snoise(vec3(val,loopTime * speed));
    //get noise from 1 loopDuration in the past
    float v2 = snoise(vec3(val,(loopTime - loopDuration) * speed));
...
void main()
{
    ...
    float n = loopNoise3d(uv * noiseScale , u_time.x, loopDuration, speed);
    n = step(0.5, n/2. + 0.5);
    gl_FragColor = vec4(n, n, n, 1.0) * color + base_color * (1.0 - n);
